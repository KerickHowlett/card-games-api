...
const allowedTypes = types.map((type) => type.value.trim());
const allowedScopes = scopes.map((scope) => scope.name.trim());
...
if (exitCode === 0) {
    ...
    console.log(
        `possible scopes: ${allowedScopes.join('|')} ( if unsure use "chore" )`
    );
    console.log(
        '\nEXAMPLE: \n' +
            'feat( client ): add an option to generate lazy-loadable modules\n'
    );
}
